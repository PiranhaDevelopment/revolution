...
class LUPAREVOLUTION extends Module
{
	public function __construct()
	{
		$this->name = 'luparevolution';
		$this->description = 'bot&atilde;o lupa header';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Luparevolution_MONOLANGUAGE';
		$this->text_editor_content = 'Luparevolution_TEXTEDITOR';
		$this->content_wrapper = 'Luparevolution_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Luparevolution_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Luparevolution_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('luparevolution');
		$this->description = $this->l('bot&atilde;o lupa header');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !LUPAREVOLUTIONModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !LUPAREVOLUTIONModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = LUPAREVOLUTIONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = LUPAREVOLUTIONModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					LUPAREVOLUTIONModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					LUPAREVOLUTIONModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																LUPAREVOLUTIONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class LUPAREVOLUTIONModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'luparevolution',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( LUPAREVOLUTIONModel::createFilesTable()
				&& LUPAREVOLUTIONModel::createContentTable());
	}
